...
{
    public class CrystalBaoCaoService
    {
        public void XemReportDoanhThuNangCao(DateTime tuNgay, DateTime denNgay)
        {
            var rpt = new ReportDoanhThu();

            try
            {
                ApplyConnection(rpt);

                DateTime denNgayCong1 = denNgay.Date.AddDays(1);

                rpt.RecordSelectionFormula =
                    "{vwRpt_DoanhThu_NangCao.NgayLapHoaDon} >= DateTime(" +
                    tuNgay.Year + "," + tuNgay.Month + "," + tuNgay.Day + ",0,0,0) AND " +
                    "{vwRpt_DoanhThu_NangCao.NgayLapHoaDon} < DateTime(" +
                    denNgayCong1.Year + "," + denNgayCong1.Month + "," + denNgayCong1.Day + ",0,0,0)";

                var frm = new System.Windows.Forms.Form();
                frm.Text = "Report nâng cao doanh thu";
